...
% define our formula
% and
QMITL_Formula('phi', '(s1[t] > 0) or (s2[t] > 0)');
...
%    [val2.time; val2.X]);
val2.name = [TRACE_PATH 'or_sig01_sig02.breach.trace'];
...
plot(sig1.time, sig1.X, '-sb',...
     sig2.time, sig2.X, '-sr',...
     val2.time, val2.X, '-sg');
title('sig1 (blue), sig2 (red), (s1[t] > 0) or (s2[t] > 0) (green)');
axis([0 3 -2 4]);
grid on;

gpuacTrace = [TRACE_PATH 'or_sig01_sig02.gpuac.trace'];
...
gpuacTrace = [TRACE_PATH 'or_sig03_sig04.gpuac.trace'];
if exist(gpuacTrace, 'file')
    gpu_result = readSignal(gpuacTrace);
    subplot(2,1,2);
    plot(gpu_result.time, gpu_result.X, '-xg');
...
val2.name = [TRACE_PATH 'or_sig03_sig04.breach.trace'];
resultArray = [resultArray val2];