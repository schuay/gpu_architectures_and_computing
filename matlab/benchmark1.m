...
%% TODO assign good values to these configs
config = [...
    2    5      5       5  ;...
    5   20     20      20  ;...
...
for curConfig = config

    y2_periodes = curConfig(1,:); %2; %5
    y1_periodes_per_y2_periode = curConfig(2,:);  %5; %20
    points_per_seq = curConfig(3,:);  %100; % 1000

    fprintf('run AND benchmark for %d points\n', points_per_seq);
    ...
    writeSignal( ...
        [TRACE_PATH sprintf('bench_sig01-%d.trace', points_per_seq)],...
        [traj.time, traj.X(1,:)]);
    writeSignal(...
        [TRACE_PATH sprintf('bench_sig02-%d.trace', points_per_seq)],...
        [traj.time, traj.X(2,:)]);
    writeSignal(...
        [TRACE_PATH sprintf('bench_and_sig01-%d_sig02-%d.breach.trace', ...
            points_per_seq, points_per_seq)],...
        [ val.time ; val.X ]);
    ...
    fprintf('run EVENTUALLY benchmark for %d points\n', points_per_seq);
    ...
    writeSignal(...
        [TRACE_PATH sprintf('bench_sig03-%d.trace', points_per_seq)],...
        [ traj.time ; traj.X ]);
    writeSignal(...
        [TRACE_PATH sprintf('bench_ev_sig03-%d.breach.trace',...
            points_per_seq)],...
        [ val.time ; val.X ]);
