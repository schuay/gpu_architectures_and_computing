...
\end{itemize}
\note{
* Primitive operators, everything else can be composed with them.
* Couldn't parallelize UNTIL, see next slide.
* Test suite/benchmarks considered more important
* Parser integration/optimization not done due to time constraints.
}
\end{frame}
...
\end{itemize}
\note{
* It took us a lot of time to figure breach out.
* As expected: some operators are hard/impossible to parallelize
* Operators often very complex, many special cases when signals not equally sampled.
* No good way to parallelize running maximum -> need to calculate for each window
* Strange floating point results, bad compiler support.
}
\end{frame}
...
\note{
* Problems took a lot of time -> no time for more than operators
* AND relatively hard because of intersections
}
\end{frame}
