...
#define OR_TEST(name, flhs, frhs, fexpected) \
START_TEST(name) \
{ \
    sigpt_t *a, *b, *c; \
 \
 ...
 \
    ...
    int nout; \
 \
    stl_or(&lhs[0], lhs.size(), &rhs[0], rhs.size(), &out, &nout); \
 \
 ...
 \
    ...
    fail_unless(c_n != -1); \
 \
    fail_unless(sigcmp(c, c_n, host_out.data(), host_out.size()) == 0); \
 \
...
OR_TEST(test_sig1, "sig01.trace", "sig02.trace", "or_sig01_sig02.breach.trace")
OR_TEST(test_sig2, "sig03.trace", "sig04.trace", "or_sig03_sig04.breach.trace")
